...
if "%1"=="" (
    ...
    podman exec webp-migrator-wordpress wp plugin status webp-safe-migrator --allow-root
) else (
    echo Checking %1 plugin status...
    podman exec webp-migrator-wordpress wp plugin status %1 --allow-root
)
goto end
...
if "%1"=="" (
    ...
    podman exec webp-migrator-wordpress wp plugin activate %1 --allow-root
)
goto end
...
if "%1"=="" (
    ...
    podman exec webp-migrator-wordpress wp plugin deactivate webp-safe-migrator --allow-root
) else (
    echo Deactivating %1 plugin...
    podman exec webp-migrator-wordpress wp plugin deactivate %1 --allow-root
)
goto end
...
echo ------------------------
podman exec webp-migrator-wordpress wp db check --allow-root
echo.
echo Database Size:
